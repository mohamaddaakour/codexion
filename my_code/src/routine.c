...
void *coder_routine(void *arg) {
	...
	while (!get_stop(coder->sim)) {
		...
		pthread_mutex_lock(&coder->comile_count_mutex);
		coder->compile_count++;
		pthread_mutex_unlock(&coder->comile_count_mutex);
...
void *monitor_routine(void *arg) {
	...
	while (!get_stop(sim)) {
		...
		while (i < sim->number_of_coders) {
			pthread_mutex_lock(&sim->coders->last_compile_mutex);
			long long last = sim->coders[i].last_compile_start;
			pthread_mutex_unlock(&sim->coders->last_compile_mutex);

			// if a coder passed the time to burnout without compiling he will burn out
			// and the simulation will stop
			if ((get_time_in_ms() - last) > sim->time_to_burnout) {
			...
			if (sim->number_of_compiles_required == -1 || sim->number_of_compiles_required > sim->coders[i].compile_count) {
				all_done = 0;
			}
